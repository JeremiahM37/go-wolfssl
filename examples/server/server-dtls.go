...
func main() {
    ...
    ctx := wolfSSL.WolfSSL_CTX_new(wolfSSL.WolfDTLSv1_2_server_method())
    if ctx == nil {
    ...
    udpAd, _ := net.ResolveUDPAddr(CONN_TYPE, CONN_HOST+":"+CONN_PORT)
    /* Listen for incoming connections */
    conn, err := net.ListenUDP(CONN_TYPE, udpAd)
    if err != nil {
    ...
    if ret != wolfSSL.WOLFSSL_SUCCESS {
        ...
        e:= wolfSSL.WolfSSL_get_error(ssl, 0) 
       // who :=  wolfSSL.WolfSSL_ERR_reason_error_string(e)
        fmt.Println(" WolfSSL_accept error ",e )
        os.Exit(1)
    } else {
